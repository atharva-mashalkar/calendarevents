...
module.exports = (app) => {
	...
	app.get('/events/:Email' , (req, res) => {
		...
		function authorize(credentials, callback) {
		  const {client_secret, client_id, redirect_urls} = credentials.web;
		  const oAuth2Client = new google.auth.OAuth2(
		      client_id, client_secret, redirect_urls[0]);
		  ...
		  (err, res) => {
		    ...
		    async function main() {
				...
				let info = await transporter.sendMail({
					...
					attachments:[
						{
							filename:'MyUpcomingEvents.json',
							path:'./MyUpcomingEvents.json'
						}
					],
