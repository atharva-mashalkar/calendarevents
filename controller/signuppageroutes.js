...
module.exports = (app) => {
	...
	app.post('/' , (req,res) => {
		...
		if(errors.length > 0){
			res.render('home', {errors,Email});
		}
		else{
			EmailUser.findOne({Email : req.body.Email},function(err,data){
    			if(err) throw err
    			if(data !== null){
    				...
    				res.redirect('/events/:'+ data.Email);
    					...
    					EmailUser.deleteOne({Email : req.body.Email},(err,res1) => {
							if(err) throw err;
							console.log('Unverified User Deleted');
							const RandomNumber = Math.random()*100000000000000000;
							const IsVerified = 'false';
							const newEmailUser = new EmailUser({Email, IsVerified , RandomNumber});
	...
	app.get('/verifyemail/:random', (req,res) => {
		EmailUser.findOne({RandomNumber : req.params.random.slice(1)},function(err,data){
    			if(err) throw err
    			else if(data != null){
	    			data.IsVerified = 'true';
	    			EmailUser.updateOne({RandomNumber : req.params.random.slice(1)},data,(err,data2) => {
					if(err) throw err;
	...
	app.get('/verification/:Email', (req,res) => {
		EmailUser.findOne({Email : req.params.Email.slice(1)},function(err,data){
    			if(err) throw err
    			res.render('verificationpage',{data : data})
			});
	})
